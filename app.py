...
@app.route("/admin/login", methods=["GET", "POST"])  # <- Add this
...
def admin_home():
    total_books = db.books.count_documents({})  # remove status filter
    total_members = db.members.count_documents({})  # remove status filter
    active_transactions = db.transactions.count_documents({"status": 1})  # keep this if 1 is valid
    ...
    if location_id:
        books = db.books.aggregate([
            ...
            { "$unwind": "$store" },
            {
                "$match": {
                    "status": True,
                    "img_file_name": { "$exists": True, "$nin": ["", None] }
                }
            },
            { "$sort": { "_id": -1 } }
        ])
...
# Member check-in-from-reserved
@app.route("/member/check-in-reserved", methods=["POST"])
def member_check_in_reserved_book():
    reserve_id = request.form.get("reserve_id")

    if not reserve_id:
        return "Missing reservation ID", 400

    reserved_book = db.reserved_books.find_one({"_id": ObjectId(reserve_id)})
    if not reserved_book:
        return "Reservation not found", 404

    book_id = reserved_book["book_id"]
    member_id = reserved_book["member_id"]
    check_in_date = datetime.today()
    due_date = check_in_date + timedelta(days=14)

    values = {
        "book_id": ObjectId(book_id),
        "checkin_location_id": None,
        "member_id": ObjectId(member_id),
        "check_in_date": check_in_date,
        "due_date": due_date,
        "check_out_date": "",
    ...
    db.transactions.insert_one(values)
    db.reserved_books.delete_one({"_id": ObjectId(reserve_id)})

    flash("Book Checked-In from reservation successfully", "success")
    return redirect(url_for("member_view_reserved_books"))
